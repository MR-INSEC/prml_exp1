    ...
    def get_classconditional_probability(self, x, dataset, h_result):
        ...
        for i in range(dataset.num):
            ...
            for index_name in dataset.data.keys():
                ...
                try:
                    XX = float(x[index_name] - x_i) / float(index_h[index_name])
                except:
                    print('=' * 10, 'BUG in Probability_density_estimation.py', '=' * 10)
                    traceback.print_exc()

                    print(f'x: {x}\n index_name - dataset.data.keys()[0]: {index_name - dataset.data.keys()[0]}\n float(index_h[index_name]: {float(index_h[index_name])}',
                          f'x_i: {x_i}')
